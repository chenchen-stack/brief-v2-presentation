...
const Store = {
  ...
  /** 五大核心 Agent + 业务操作型助手 */
  agents: [
    { id: 'forecast', name: '现金流预测 Agent', avatarKey: 'forecast', desc: '日/周/月滚动预测、多场景模拟、偏差追踪；输出附数据来源与假设，可解释可追溯', tools: ['文件系统','代码与终端','图表与报表'], skills: ['资金流预测','分析预测','情景模拟'] },
...
const pageLabels = { workbench:'业务工作台', dashboard:'总览看板', cashflow:'资金流管理', analysis:'分析预测', plan:'资金计划', fx:'外汇敞口', basedata:'基础数据', integration:'数据集成', platform:'中台能力', payment:'付款排程', arap:'往来款管理', alerts:'资金预警中心', cockpit:'管理驾驶舱', chat:'对话' };
...
function localFallback(msg) {
  ...
  if (lm.includes('分析') || lm.includes('预测')) {
    ...
    return '正在为您**运行分析预测**…\n\n分析基于当前资金流数据，使用复合周期引擎（天/周/月/季）生成预测报告。\n\n---\n\n**预测范围**：未来14个时段\n**模型类型**：复合周期引擎\n**数据基础**：' + (s.record_count||0) + '条资金流记录\n\n预测结果将在右侧面板中展示。' + navCmd('analysis');
  }
...
const WB_TREASURY_TILES = [
  ...
  { page: 'analysis', label: '分析预测', icon: 'trend' },
      ...
      <div class="workbench-section">
        ...
        <div class="scene-card-grid">
          <div class="scene-card scene-card--primary" role="button" tabindex="0" id="wbCardTreasury">
            ...
            <div class="scene-card-desc">含总览、驾驶舱、付款排程、往来款、资金流、分析预测、计划、外汇、基础数据、集成与预警等完整能力。</div>
...
function renderSkills() {
  const categories = [
    ...
    { name: '资金管理', catIcon: 'wallet', count: 10, skills: [
      ...
      { name: '分析预测', desc: '头寸递推、科目×时间段矩阵', on: true },
  ...
  pg.innerHTML = `
    ...
    <div class="page-scroll">
      ...
      <div class="kpi-row">
        ...
        <div class="kpi-card" onclick="navigateTo('analysis')"><div class="kpi-label">预测偏差</div><div class="kpi-value">—</div><div class="kpi-sub">联动分析预测模块</div></div>
      </div>
    ...
    <div class="page-scroll">
      ...
      <div class="card"><div class="card-header"><div class="card-title">业务闭环快捷入口</div></div>
        <div class="card-body dash-quick-actions">
          ...
          <button type="button" class="btn dash-quick-btn" onclick="navigateTo('analysis')"><span class="dash-quick-ico">${uiIcon('trend')}</span>分析预测</button>
...
/* ════════════════════════════════════════════
   User Menu & Settings
   ════════════════════════════════════════════ */
/** 账户菜单：fixed + 贴头像行右侧；挂 body 避免 #accioApp overflow 裁切 */
function syncUserMenuPanelPosition() {
  ...
  const w = Math.min(prefW, vw - left - 8, vw - 16);
...
function initUserMenu() {
  ...
  const sidebarEl = document.getElementById('accioSidebar');
  ...
  const setUserMenuOpen = (isOpen) => {
    ...
    sidebarEl?.classList.toggle('user-menu-open', isOpen);
    if (isOpen) {
      if (footerEl && menu.parentElement !== document.body) {
        document.body.appendChild(menu);
      }
      requestAnimationFrame(() => requestAnimationFrame(() => syncUserMenuPanelPosition()));
    } else {
  ...
  const reflowUserMenu = () => {
    if (menu.classList.contains('open')) syncUserMenuPanelPosition();
  };
  window.addEventListener('resize', reflowUserMenu);
  window.addEventListener('scroll', reflowUserMenu, true);
