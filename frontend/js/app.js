...
(async function () {
  ...
  function renderDashboard() {
    ...
    document.getElementById('dash-kpi').innerHTML =
      ...
      kpi('资金流预测', 'MVP', '滚动与关键节点', null, 'liquidity');
  ...
  function _updateDegradationUI() {
    ...
    var fetchBtn = document.getElementById('cf-btn-fetch');
    var bankStatus = AppData.systemHealth.bank ? AppData.systemHealth.bank.status : 'ok';
    if (fetchBtn) {
      if (bankStatus !== 'ok') {
        fetchBtn.textContent = '⬇ 获取数据 (降级)';
        fetchBtn.title = '银行接口' + (bankStatus === 'degraded' ? '降级' : '离线') + '；仍可先一键拉取（演示缓存）。Shift+点击可选条件';
      } else {
        fetchBtn.textContent = '⬇ 获取数据';
        fetchBtn.title = '一键拉取；Shift+点击可选条件';
      }
    }
  }
  ...
  /** 集成取数：unit 空字符串表示全部单位；日期可空 */
  function _cfRunIntegrationsFetch(unit, dateStart, dateEnd, sys) {
    var bankStatus = AppData.systemHealth.bank ? AppData.systemHealth.bank.status : 'ok';
  ...
  function cfOpenFetchModal() {
    ...
    openModal('获取数据（可选条件）',
      '<p style="margin-bottom:12px;">将根据 <strong>' + rules.length + '</strong> 条有效映射规则拉取；留空表示与「一键获取」相同口径。</p>' +
      '<div class="form-group"><label>单位</label><select class="form-select" id="m-fetch-unit"><option value="">全部单位</option><option>总部</option><option>华东子公司</option><option>华南子公司</option></select></div>' +
      ...
      '<div class="form-group"><label>来源系统</label><select class="form-select" id="m-fetch-sys"><option>资金管理系统</option></select></div>',
      function () {
        ...
        closeModal();
        _cfRunIntegrationsFetch(unit, dateStart, dateEnd, sys);
      });
  }

  // 获取数据：单击 = 一键拉取；Shift+单击 = 弹窗指定条件
  _domOn('cf-btn-fetch', 'click', function (ev) {
    var rules = (AppData.mappingRules || []).filter(function (r) { return r.valid; });
    if (!rules.length) { Toast.warn('暂无有效映射规则，请先到【数据集成】配置'); return; }
    if (ev.shiftKey) {
      cfOpenFetchModal();
  ...
  _domOn('an-btn-export', 'click', function () {
    ...
    var a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = '分析预测报告_' + new Date().toISOString().slice(0, 10) + '.csv';
    a.click();
    Toast.success('报告已导出');
  });
  ...
  function renderAnalysisAiReport(result) {
    ...
    (async function () {
      ...
      _anAiTypewriterCancel = _anTypewriterPlain(el, text, seq, function () {
        ...
        if (tagsEl) {
          tagsEl.innerHTML =
            '<span class="muted" style="display:block;font-size:11px;margin-bottom:8px;">点击标签将打开资金流管理并自动筛选（闭环）</span>' +
  ...
  if (_planFromAn) _planFromAn.addEventListener('click', function () {
    if (!AppData.analysisResult) { Toast.warn('请先到【分析预测】页面运行分析'); return; }
  ...
  /** 最近一次「预测」返回的 mvp-forecast，供右侧自然语言追问绑定引擎摘要 */
  var _liqLastForecastRes = null;
  ...
  function _liqOpenMonthSubjectModal(tr) {
    ...
    function renderRows(items, source) {
      ...
      if (!items || !items.length) {
        wrap.innerHTML = '<p class="muted" style="font-size:13px;margin:0;">暂无科目明细数据</p>' +
          '<p class="muted" style="font-size:11px;margin:8px 0 0;">可在「资金流管理」录入带科目分录的已确认单据后重试。</p>' + footer;
  ...
  function _liqRiskDefaultVisiblePrompt() {
    return '请根据左侧引擎快照，解读滚动预测结果：是否触及或接近警戒线？预警样本大致落在哪些日期？请给出流动性结论与可执行的审慎建议。';
  }

  function _buildLiqRiskPayloadObject(res) {
    var labels = res.labels || [];
    var alerts = res.alert_indices || [];
  ...
  function _liqRiskBubbleUserHtml(text) {
    return (
      '<div class="liq-risk-bubble liq-risk-bubble--user">' +
      '<div class="liq-risk-bubble__label">您</div>' +
      '<div class="liq-risk-bubble__text">' +
      _liqEscHtml(text) +
      '</div></div>'
    );
  }

  function _liqRiskBubbleAssistantShell(innerHtml) {
    return (
      '<div class="liq-risk-bubble liq-risk-bubble--assistant">' +
      '<div class="liq-risk-bubble__label">亿流 Copilot</div>' +
      '<div class="liq-risk-bubble__body">' +
      innerHtml +
      '</div></div>'
    );
  }
  ...
  function _fillLiqRiskStatic(res, riskEl) {
    ...
    var inp = document.getElementById('liq-risk-input');
    if (inp) inp.value = _liqRiskDefaultVisiblePrompt();
    el.innerHTML =
      _liqRiskBubbleUserHtml(_liqRiskDefaultVisiblePrompt()) +
      _liqRiskBubbleAssistantShell('<div class="liq-risk-ai-md">' + body + '</div>');
  }

  function _buildLiqRiskAiUserMessage(res, visibleQuestion) {
    var head = String(visibleQuestion || '').trim() || _liqRiskDefaultVisiblePrompt();
    var payload = _buildLiqRiskPayloadObject(res);
    return (
      head +
      '\n\n【以下为「资金流滚动预测」引擎摘要 JSON，请严格据此引用数字与日期，勿编造】\n' +
  ...
  async function _fillLiqRiskFromForecast(res) {
    ...
    var q0 = _liqRiskDefaultVisiblePrompt();
    risk.innerHTML =
      _liqRiskBubbleUserHtml(q0) +
      _liqRiskBubbleAssistantShell('<p class="muted" style="margin:0;font-size:13px;">正在生成 Copilot 解读…</p>');
    try {
      var text = await AI.chatCompletionOneShot(
        AI.LIQUIDITY_RISK_ALERT_SYSTEM_PROMPT,
        _buildLiqRiskAiUserMessage(res, q0),
        { max_tokens: 1400 }
      );
      ...
      risk.innerHTML =
        _liqRiskBubbleUserHtml(q0) +
        _liqRiskBubbleAssistantShell(
          '<div class="liq-risk-ai-md">' +
            html +
            '</div>' +
            '<p class="liq-risk-inline-foot">以上由大模型根据当前预测摘要生成，仅供参考；审批与头寸决策请以主台规则与司库复核为准。</p>'
        );
  ...
  async function _liqRiskOnSend() {
    var res = _liqLastForecastRes;
    if (!res) {
      if (typeof Toast !== 'undefined') Toast.warn('请先点击上方「预测」生成引擎快照');
      return;
    }
    if (typeof AI === 'undefined' || !AI.chatCompletionOneShot || !AI.LIQUIDITY_RISK_ALERT_SYSTEM_PROMPT) {
      if (typeof Toast !== 'undefined') Toast.warn('未配置 AI 或缺少流动性解读模型');
      return;
    }
    var inp = document.getElementById('liq-risk-input');
    var q = inp && inp.value.trim();
    if (!q) q = _liqRiskDefaultVisiblePrompt();
    var box = document.getElementById('liq-risk-alerts');
    if (!box) return;
    var loadId = 'liq-risk-pend-' + Date.now();
    box.insertAdjacentHTML(
      'beforeend',
      _liqRiskBubbleUserHtml(q) +
        _liqRiskBubbleAssistantShell('<p class="muted" style="margin:0;font-size:13px;" id="' + loadId + '">正在生成…</p>')
    );
    box.scrollTop = box.scrollHeight;
    try {
      var text = await AI.chatCompletionOneShot(
        AI.LIQUIDITY_RISK_ALERT_SYSTEM_PROMPT,
        _buildLiqRiskAiUserMessage(res, q),
        { max_tokens: 1400 }
      );
      var pend = document.getElementById(loadId);
      var inner =
        text && String(text).trim()
          ? (typeof AI.renderMarkdown === 'function'
              ...
              : '<p>' + String(text).replace(/</g, '&lt;') + '</p>')
          : '<p class="muted">（无返回）</p>';
      if (pend && pend.parentElement) {
        pend.parentElement.innerHTML =
          '<div class="liq-risk-ai-md">' +
          inner +
          '</div>' +
          '<p class="liq-risk-inline-foot">仅供参考，请以司库复核与主台规则为准。</p>';
        if (typeof AI.bindInlineLinks === 'function') AI.bindInlineLinks(pend.parentElement);
      }
    } catch (e) {
      console.warn('_liqRiskOnSend', e);
      var pend = document.getElementById(loadId);
      if (pend && pend.parentElement) {
        pend.parentElement.innerHTML =
          '<p class="muted" style="margin:0;">生成失败：' + _liqEscHtml(e.message || String(e)) + '</p>';
      }
    }
    box.scrollTop = box.scrollHeight;
  }
  ...
  async function _runLiqMvpForecast(opts) {
    ...
    if (!backendOk) {
      ...
      _liqLastForecastRes = null;
      if (riskOff) {
        _renderLiqRiskMetricsOffline();
        var inp0 = document.getElementById('liq-risk-input');
        if (inp0) inp0.value = _liqRiskDefaultVisiblePrompt();
        riskOff.innerHTML =
          _liqRiskBubbleUserHtml(_liqRiskDefaultVisiblePrompt()) +
          _liqRiskBubbleAssistantShell(
            '<div class="liq-risk-ai-md"><ul style="margin:0;padding-left:18px;">' +
              '<li><strong>演示预警：</strong>应收/应付超期示例。</li>' +
              '<li>连接后端后按真实余额与警戒线计算；连接后可使用下方输入框追问。</li>' +
            '</ul></div>'
          );
      }
      return;
    ...
    try {
      ...
      if (hint) hint.textContent = '提示：在「资金流管理」录入或同步跨月单据后再试。';
      ...
      if (riskErr) {
        ...
        var inpE = document.getElementById('liq-risk-input');
        if (inpE) inpE.value = '';
        riskErr.innerHTML =
          _liqRiskBubbleAssistantShell(
            '<p class="muted" style="margin:0;">无法加载后端预测。请先在「资金流管理」入库至少 2 个月流水后再点「预测」，再使用右侧对话。</p>'
          );
      }
  ...
  function renderLiquidityMvp() {
    ...
    if (!_liqUiBound) {
      ...
      _domOn('liq-risk-send', 'click', function () {
        _liqRiskOnSend();
      });
      _domOn('liq-risk-input', 'keydown', function (e) {
        if (e.key === 'Enter' && !e.shiftKey) {
          e.preventDefault();
          _liqRiskOnSend();
        }
      });
      _domOn('liq-risk-confirm', 'click', function () {
        if (!_liqLastForecastRes) {
          if (typeof Toast !== 'undefined') Toast.warn('请先点击上方「预测」生成引擎快照');
          return;
        }
        var res = _liqLastForecastRes;
        var unit = res.unit != null && String(res.unit).trim() ? String(res.unit) : '全集团';
        var hz = res.horizon_days != null ? String(res.horizon_days) : '—';
        var note = res.method_note ? String(res.method_note).slice(0, 160) : '—';
        openModal(
          '上报稿预览（演示）',
          '<p style="margin:0 0 10px;font-size:13px;line-height:1.65;color:var(--text-secondary);">以下为基于<strong>当前左侧快照</strong>生成的流动性说明稿摘要页，<strong>未写入</strong> ERP / 总账，仅供演示人确认流。</p>' +
            '<div style="font-size:12px;line-height:1.7;border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--bg-muted,#f8f9fb);">' +
            '<div><strong>单位</strong>：' +
            _liqEscHtml(unit) +
            '</div>' +
            '<div><strong>预测天数</strong>：' +
            _liqEscHtml(hz) +
            ' 天</div>' +
            '<div style="margin-top:8px;"><strong>引擎摘要</strong>：' +
            _liqEscHtml(note) +
            (String(res.method_note || '').length > 160 ? '…' : '') +
            '</div>' +
            '<p style="margin:10px 0 0;font-size:11px;color:var(--text-muted);">留痕：与 mvp-forecast 快照同源 · 确认后可在真实环境中导出 PDF / 推送审批（需对接）。</p></div>',
          null,
          true
        );
        if (typeof Toast !== 'undefined') Toast.success('已生成预览（演示）');
      });
      _domOn('liq-risk-reject', 'click', function () {
        var inp = document.getElementById('liq-risk-input');
        if (typeof Toast !== 'undefined') Toast.info('已驳回当前草稿：请在下框修改说明后再次「发送」');
        if (inp) {
          inp.focus();
          try {
            inp.select();
